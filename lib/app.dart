...
class _LaunchDeciderState extends State<_LaunchDecider> {
  ...
  Future<void> _route() async {
    // Run checks in parallel to minimize splash time
    ...
    final PermissionStatus cameraStatus = results[1] as PermissionStatus;
    
    // Explicitly check for onboarding status
    final onboardingDone = prefs.getBool('onboarding_completed') ?? false;

    if (!mounted) return;
    
    // Navigate immediately in the next frame
