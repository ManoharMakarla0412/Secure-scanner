...
class _InitialPermissionsScreenState extends State<InitialPermissionsScreen> with WidgetsBindingObserver {
  ...
  void initState() {
    super.initState();
    FirebaseAnalytics.instance.logScreenView(screenName: 'InitialPermissionsScreen');
  ...
  Future<void> _requestPermission(Permission permission) async {
    FirebaseAnalytics.instance.logEvent(
      name: 'permission_request_onboarding',
      parameters: {'permission': permission.toString()},
    );
    if (permission == Permission.photos && Platform.isAndroid) {
      final status = await Permission.photos.request();
      FirebaseAnalytics.instance.logEvent(
        name: 'permission_result_onboarding',
        parameters: {'permission': 'photos', 'status': status.name},
      );
      if (!status.isGranted) {
        final storageStatus = await Permission.storage.request();
        FirebaseAnalytics.instance.logEvent(
          name: 'permission_result_onboarding',
          parameters: {'permission': 'storage', 'status': storageStatus.name},
        );
      }
    } else {
      ...
      FirebaseAnalytics.instance.logEvent(
        name: 'permission_result_onboarding',
        parameters: {'permission': permission.toString(), 'status': status.name},
      );
    }
    _checkPermissions();
  }
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: isGranted ? SecureScanTheme.accentBlue.withValues(alpha: 0.05) : Colors.grey[50],
          borderRadius: BorderRadius.circular(20),
          border: Border.all(
