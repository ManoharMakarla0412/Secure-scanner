...
class _ScanScreenQRState extends State<ScanScreenQR>
  ...
  Future<void> _initializeCamera() async {
    // Check permission status first (Fast check)
    final status = await Permission.camera.status;
    
    if (status.isGranted) {
      // Direct to Camera (Retention Fix)
      await _cameraController.start();
      if (mounted) setState(() {});
    } else {
      // Wait for frame then request (Avoids blocking UI thread during heavy startup)
  ...
  Future<void> _handleBarcode(Barcode barcode, Uint8List? imageBytes) async {
    ...
    // Security check for risky URLs (Non-blocking dialog)
    if (type == QrType.url) {
      final isRisky = raw.contains('bit.ly') || raw.contains('t.co') || raw.contains('tinyurl.com') || raw.contains('goo.gl');
      if (isRisky) {
        _showIssueDialog('Security Warning: This shortened URL might lead to a suspicious website.');
      }
    }

    // Process scan details (Save image and history item)
    // We do this in a single batch to avoid multiple awaits slowing down UI
    Future(() async {
      ...
      if (imageBytes != null && _docsDir != null) {
        ...
        try {
          File(savedImagePath).writeAsBytes(imageBytes); // Trigger write but don't await blocking
        } catch (e) {
      ...
      // 1. Initial navigation to the results screen immediately (Retention Fix: No delay for users)
      if (!mounted) return;
      ...
      Navigator.push(
        ...
        // Once they return from the result screen, we clean up the processing state
        if (mounted) setState(() => _isProcessing = false);
      });

      // 2. Trigger ad non-blocking (Revenue Optimization)
  ...
  Future<void> _scanFromGallery() async {
    ...
    FirebaseAnalytics.instance.logEvent(name: 'scan_gallery_start');
  ...
  Future<void> _handleBarcodeMLKit(HistoryItem historyItem, Uint8List imageBytes) async {
    ...
    // Navigate immediately (Retention Fix)
    // We process saving and ads in the background
    Future(() async {
      String? savedImagePath;
      if (_docsDir != null) {
        ...
        savedImagePath = '$_docsDir/$fileName';
        try {
          File(savedImagePath).writeAsBytes(imageBytes);
        } catch (_) {}
      }
